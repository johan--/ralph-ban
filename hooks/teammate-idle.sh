...
# Exit 2 + stderr = keep working. Exit 0 + suppressOutput = allow idle silently.
trap 'exit 0' ERR
set -uo pipefail
...
if ! db_exists; then echo '{"suppressOutput": true}'; exit 0; fi
...
[ -z "$teammate" ] && { echo '{"suppressOutput": true}'; exit 0; }
